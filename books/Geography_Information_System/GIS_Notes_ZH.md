...
* **曲率半径：** 椭球面上每一点的曲率半径是不同的。主要的曲率半径包括：
  ...
  * **卯酉圈曲率半径 (
    $$
    N$$)：** 沿卯酉圈（垂直于子午线的法截面）方向的曲率半径，也称法截弧曲率半径。平行于卯酉圈的纬线圈半径
    $$
...
* **局部平均球面半径 (
  ...
  R_{\text{localsphere}} = \sqrt{\rho N}$$。
* **适用性：** 椭球长短半轴差异约21公里，在小范围内用球面近似误差较小。例如，

  $$
  \frac{N-\rho}{N} \le \frac{1}{150}
  $$
...
@GISnote.md 这个笔记里面的一些数学公式并没有很好的展示在web端.请逐个帮我校核与修改.使其可以直观的展示在web网页中.目前markdown中是可以直观的展示的.我希望你做一个备份,然后用备份文件去尝试进行调整,这样即使没能改正确,也不影响我原先的版本.
