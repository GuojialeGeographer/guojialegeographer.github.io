...
这份笔记旨在系统化地呈现 SHUD 模拟系统的理论、方法和实际应用。通过这份笔记，您将学习如何构建、运行、率定和分析 SHUD 模型，并了解其在水文研究和应用中的价值。
...
* **示例:** SWAT (Soil and Water Assessment Tool)(半分布式), VIC (Variable Infiltration Capacity), **SHUD (Simulator for Hydrologic Unstructured Domains)**, MIKE SHE, TopModel (半分布式)。
...
* **功能:** 将 SHUD 建模的多个步骤 (数据准备、预处理、模型构建、初步运行、参数文件生成等) 封装成可配置的自动化脚本。用户通过修改一个核心配置文件 (`.autoshud.txt`)，即可快速完成一个流域的 SHUD 模型搭建。
...
* **结构:** 包含两个主要的表格块。
  ...
  1. **表格块 1: 三角形单元信息 (Cell Information)**
     ...
     * **文件头:**
       ...
       ```
     * **数据表 (每行代表一个三角形单元):**| 列名      | 含义                                 | 示例值 | 单位 | 备注                                                                  |
       | :-------- | :----------------------------------- | :----- | :--- | :-------------------------------------------------------------------- |
...
* **结构:** 一个表格块。
  ...
  * **文件头:**
    ...
    ```
  * **数据表 (每行代表一个被三角形切割出的河道线段):**| 列名       | 含义                      | 示例值 | 单位 | 备注                                                                     |
    | :--------- | :------------------------ | :----- | :--- | :----------------------------------------------------------------------- |
...
* **结构:** 一个表格。
  ...
  * **数据表 (每行代表一种土壤类型):**| 列名 (示例)         | 含义                                       | 典型单位   | 备注                                                                                      |
    | :------------------ | :----------------------------------------- | :--------- | :---------------------------------------------------------------------------------------- |
...
* **结构:** 与 `.para.soil` 类似，一个表格。
  ...
  * **数据表 (每行代表一种地质类型):**| 列名 (示例)     | 含义                             | 典型单位   | 备注                                                                                                     |
    | :-------------- | :------------------------------- | :--------- | :------------------------------------------------------------------------------------------------------- |
...
* **结构:** 一个表格。
  ...
  * **数据表 (每行代表一种土地利用类型):**| 列名 (示例)                | 含义                                     | 典型单位   | 备注                                                                       |
    | :------------------------- | :--------------------------------------- | :--------- | :------------------------------------------------------------------------- |
...
* **关联的 `.csv` 气象驱动数据文件结构 (每个文件对应一个站点或格点):**
  ...
  * **数据表 (每行代表一个时间步长):**| 列名 (示例)                    | 含义                            | 典型单位              | 备注                                                               |
    | :----------------------------- | :------------------------------ | :-------------------- | :----------------------------------------------------------------- |
...
AutoSHUD 是一套基于 *R* 语言和 rSHUD 包开发的自动化脚本集合。它的核心目标是将上述复杂且耗时的建模前处理流程自动化。用户通过配置一个核心的文本文件 (`.autoshud.txt`)，指定输入数据源、建模参数和输出路径，AutoSHUD 即可自动执行从数据下载、预处理、网格生成到 SHUD 输入文件生成的绝大部分步骤。
...
* **主目录:**
  ...
  * `.autoshud.txt` (或类似名称的配置文件): **核心配置文件**，用户通过修改此文件来控制整个建模流程。
* **子目录:**
...
### 8.2.2. 核心配置文件: `.autoshud.txt` 文件详解

`.autoshud.txt` 文件是用户与 AutoSHUD 交互的主要接口。通过修改此文件中的参数，可以定制化建模流程以适应不同的研究区域和需求。
...
* **本地数据优先:** 如果用户在 `.autoshud.txt` 中为 `WBD`, `RIV`, `DEM` 等参数提供了具体的本地文件路径，AutoSHUD 将优先使用这些本地文件。
...
> **图表建议 8.3:** 插入一份完整的 `.autoshud.txt` 文件示例，并对每一行或关键参数块进行详细注释，解释其含义、可选值和对建模的影响。
> **图表建议 8.4:** 插入一个决策流程图，展示 AutoSHUD 如何根据 `.autoshud.txt` 中的参数 (特别是数据源选择和本地文件路径) 决定数据获取和处理的逻辑。
...
* **功能:**
  ...
  * **读取 `.autoshud.txt` 配置文件:** 将配置文件中的参数解析并加载到 *R* 的工作环境中，作为全局变量供后续脚本使用。
...
* **实操:**
  1. [确保 `GetReady.R` 已成功执行，并且 `.autoshud.txt` 中正确配置了 `WBD`, `RIV`, `DEM` 的路径。]
...
* **功能:**
  * **处理全球/区域数据集:** 根据 `.autoshud.txt` 中的 `SOIL_DATA`, `LC_DATA`, `FORCING_DATA` 等配置，调用相应的子脚本 (`SubScript/` 目录下) 来获取、裁剪、重采样和格式化土壤、土地利用和气象驱动数据。
...
* **实操:**
  ...
  2. [确保 `.autoshud.txt` 中正确配置了所需数据源及其路径。]
...
* **功能:**
  * **流域离散化与网格生成:**
    * 利用 `rSHUD::triangulate` 等函数，基于预处理好的 DEM、河网以及 `.autoshud.txt` 中指定的网格控制参数 (`NCELL`, `MAX_AREA`, `MIN_ANGLE`, `TOL_WBD`, `TOL_RIV`) 生成非结构化三角形网格。
    * 确定河段与三角形单元的相交关系。
  * **属性与参数赋予:**
    * 将 Step2 生成的土壤、土地利用属性空间数据与生成的三角形网格叠加，为每个单元赋予属性索引。
    * 根据属性索引，从内部或外部的参数查找表 (`Table/` 目录或 rSHUD 内置) 中查找并分配水力学参数。
  * **SHUD 输入文件生成:**
    * 将所有几何信息、拓扑关系、属性索引、水力学参数、模型运行配置 (基于默认值和 `.autoshud.txt` 的部分设置)、默认初始条件以及格式化后的时间序列数据列表，按照 SHUD 要求的格式写入到最终的输入文件集 (`.sp.*`, `.cfg.*`, `.para.*`, `.tsd.forc` 等)。
* **实操:**
...
第八章节详细介绍了 AutoSHUD 自动化建模工具的脚本结构、核心配置文件 `.autoshud.txt` 的各项参数，以及通过依次执行 `GetReady.R` 和 `Step1` 到 `Step3` 脚本来完成从原始数据到 SHUD 模型输入文件的整个前处理和构建过程。理解 AutoSHUD 的工作流程和配置方法，将极大地提升 SHUD 建模的效率和规范性。

**思考题:**

1. 在 `.autoshud.txt` 中，气象驱动数据源参数 `FORCING_DATA` 和 `FORCING_PATH` 是如何协同工作的？如果我想使用我自己准备的、非标准格式的本地气象站点数据，AutoSHUD 是否能直接处理？如果不能，我应该如何修改或扩展 AutoSHUD 的功能？
2. AutoSHUD 生成的 SHUD 输入文件中，水力学参数 (`.para.soil`, `.para.geol`, `.para.lc`) 的值是如何确定的？它们是基于全球默认值，还是会根据用户提供的土壤和土地利用数据进行更精细的匹配？
3. 如果 `Step3_BuidModel.R` 脚本在生成三角形网格时耗时过长或失败，可能是什么原因？应如何调整 `.autoshud.txt` 中的网格控制参数 (`NCELL`, `MAX_AREA`, `MIN_ANGLE` 等) 来优化网格生成过程？
...
2. **非结构化网格生成:**
   ...
   * 应用 `.autoshud.txt` 中定义的网格控制参数 (`NCELL`, `MAX_AREA`, `MIN_ANGLE`, `TOL_WBD`, `TOL_RIV`) 来优化网格质量和密度。
...
6. **配置文件生成:**
   * 生成默认的 `.cfg.para` (运行参数), `.cfg.calib` (校准参数基准), 和 `.cfg.ic` (初始条件) 文件。这些文件的部分内容会参考 `.autoshud.txt` 中的设置。
7. **时间序列数据索引文件生成:**
...
* `.autoshud.txt` 中正确的网格控制参数和数据路径配置。
...
2. **检查 `.autoshud.txt` 配置:** 再次确认 `.autoshud.txt` 中的网格参数 (`NCELL`, `MAX_AREA`, `MIN_ANGLE` 等) 和相关路径设置符合您的需求。
3. **执行脚本:**
   ```R
...
脚本成功运行后，在 `.autoshud.txt` 中 `DIR_OUT` 参数指定的输出目录下的 `deploy/<ProjectName>/MODEL/Input/` 文件夹中，应该能找到一套完整的 SHUD 输入文件。
...
> **图表建议 9.1:** 插入一个流程图，展示 `Step3_BuidModel.R` (或核心建模函数) 如何利用预处理数据 (DEM, 河网, 土壤图, 土地利用图, 气象数据列表) 和配置参数 (`.autoshud.txt`) 来生成 SHUD 的各类输入文件 (`.sp.*`, `.para.*`, `.cfg.*`, `.tsd.*`)。

### 9.2.6. 常见问题

* **脚本中断或报错:**
  * **原因:** 前序步骤数据缺失或格式错误；`.autoshud.txt` 配置参数不合理 (如网格参数导致无法生成有效网格)；rSHUD 包或其依赖包函数出错。
...
* **生成的网格质量差:**
  ...
  * **解决:** 调整 `.autoshud.txt` 中的网格控制参数，重新运行 `Step3`。
* **属性赋予错误:**
...
### 12.3.1. 配置 `.autoshud.txt` 文件 (确认与微调)

* 在完成数据预处理后，再次检查并确认 `.autoshud.txt` 中的网格生成参数 (`NCELL`, `MAX_AREA`, `MIN_ANGLE`, `TOL_WBD`, `TOL_RIV`)、含水层厚度 (`AQ_DEPTH`)、一级河道参数 (`RIV_WIDTH`, `RIV_DEPTH`) 等设置是否合理。
...
2. **阶段二：检查 GHDC 生成的模型，并准备在本地/HPC 运行 (可选，但推荐)**
   ...
   * **2.2. (如果需要，但不常见于此流程) 使用 AutoSHUD 进行精细化调整或重新构建：**
     ...
     * **操作：** 将 GHDC `ETV/` 目录下的核心空间数据 (DEM, 处理后的土壤图、土地利用图) 作为 AutoSHUD 的输入，配置 `.autoshud.txt`，然后运行 AutoSHUD 的 `Step1` 到 `Step3`。**此时，您需要确保 AutoSHUD 使用的气象数据处理逻辑与 GHDC 一致，或者手动替换 AutoSHUD 生成的气象驱动为 GHDC 提供的。**
